...
import { DnaIcon,Activity } from "lucide-react";
...
const GenomeCard = ({ genome }) => {
  ...
  return (
    <Link href={`/genomes/${genome.id}`} className="group block">
      <Card className="relative overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:shadow-lg border  bg-white dark:bg-gray-800 backdrop-blur-sm">
        ...
        <CardContent className="p-6 space-y-4 relative">
          <div className="flex items-start justify-between gap-4">
            <div className="space-y-1 min-w-0">
              ...
              <CardDescription className="text-gray-600 dark:text-gray-400 truncate">
                {genome.species} •{" "}
                {genome.header ? genome.header.split(" ")[0] : ""}
              </CardDescription>
            </div>
            <Badge
              variant="secondary"
              className={`shrink-0 ${getStatusColor(genome.status)}`}
            >
              {genome.status}
            </Badge>
          </div>
          ...
          <div className="flex items-center justify-between text-sm text-gray-500 dark:text-gray-400">
            ...
            <span className="text-xs">
              Modified{" "}
              {genome.lastModified && genome.lastModified !== "N/A"
                ? genome.lastModified
                : "0 days ago"}
            </span>
          </div>
